    ...
    <section className="flex max-h-full w-[1080px] max-w-full flex-col overflow-hidden rounded-[26px] border border-[#2A2A31] bg-[#141416] shadow-[0_40px_90px_rgba(0,0,0,.55)]" aria-label="MCP servers">
      ...
      <div className="rk-scroll grid min-h-0 grid-cols-1 gap-6 overflow-y-auto p-8 lg:grid-cols-[1fr_1.08fr]">
        ...
        <div className="space-y-5">
          ...
          <div><h2 className="text-[15px] font-medium text-[#ECECEE]">Configured servers</h2>{error ? <p className="mt-2 rounded-xl border border-[#6A2C37] bg-[#2A151A] p-3 text-xs text-[#F3A2AA]">{error}</p> : null}<div className="mt-3 space-y-2">{servers.length === 0 ? <p className="rounded-xl border border-dashed border-[#34343B] p-5 text-sm text-[#77777F]">No MCP servers yet.</p> : servers.map((server) => <div key={server.id} className="rounded-xl border border-[#292930] bg-[#101012] p-4"><div className="flex items-center justify-between"><span className="font-medium text-[#ECECEE]">{server.name}</span><span className="rounded-full bg-[#202536] px-2 py-1 text-[10px] uppercase text-[#AEB7FF]">{server.transport.replace("_", " ")}</span></div><p className="mt-1 text-xs text-[#77777F]">{server.endpoint ?? server.command ?? server.slug}</p><p className="mt-2 text-[11px] text-[#6E778A]">{server.hasSecret ? "Encrypted credential saved" : "No credential saved"}</p>{server.transport !== "stdio" ? <div className="mt-3 flex flex-wrap items-center gap-2"><button type="button" disabled={oauthPending === server.id} onClick={() => void connectOAuth(server)} className="rounded-lg bg-[#7785FF] px-3 py-2 text-xs font-semibold text-[#090A12] disabled:opacity-50">{oauthPending === server.id ? "Preparing sign-in…" : "Connect OAuth"}</button>{server.hasSecret ? <button type="button" disabled={oauthPending === server.id} onClick={() => void disconnectOAuth(server)} className="rounded-lg border border-[#34343B] px-3 py-2 text-xs text-[#B9B9C0]">Disconnect</button> : null}{oauthPending === server.id ? <span role="status" className="text-[11px] text-[#8E93B8]">Contacting the provider. This can take up to a minute.</span> : null}</div> : null}</div>)}</div></div>
        </div>
      </div>
    </section>
  </div>;
