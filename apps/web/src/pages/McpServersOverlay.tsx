...
export function McpServersOverlay({ onClose }: { onClose: () => void }) {
  ...
  async function refresh() {
    ...
    const assignmentLists = await Promise.all(activeBots.map((bot) => rpc.mcp.assignments.list({ botId: bot.id })));
    setServers(nextServers);
    setBots(activeBots);
    setBotAssignments(Object.fromEntries(activeBots.map((bot, index) => [
      bot.id,
      (assignmentLists[index] ?? []).map((entry) => ({ serverId: entry.serverId, allowAllTools: entry.allowAllTools, allowedTools: entry.allowedTools })),
    ])));
  }
  ...
  async function addServer() {
    ...
    try {
      ...
      await Promise.all(selectedBotIds.map((botId) => {
        const existing = (botAssignments[botId] ?? []).filter((entry) => entry.serverId !== created.id);
        return rpc.mcp.assignments.replace({ botId, assignments: [...existing, { serverId: created.id, allowAllTools: true, allowedTools: [] }] });
      }));
      await refresh();
  ...
  async function toggleAssignment(server: McpServer, botId: string) {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : "Could not update agent access"); }
  }

  async function deleteServer(server: McpServer) {
    if (confirmingDelete !== server.id) { setConfirmingDelete(server.id); return; }
    setConfirmingDelete(null);
    setError(null);
    try { await rpc.mcp.servers.remove({ id: server.id }); await refresh(); }
    catch (err) { setError(err instanceof Error ? err.message : "Could not delete MCP server"); }
  }
    ...
    <section className="flex max-h-full w-[1080px] max-w-full flex-col overflow-hidden rounded-[26px] border border-[#2A2A31] bg-[#141416] shadow-[0_40px_90px_rgba(0,0,0,.55)]" aria-label="MCP servers">
      ...
      <div className="rk-scroll grid min-h-0 grid-cols-1 gap-6 overflow-y-auto p-8 lg:grid-cols-[1fr_1.08fr]">
        ...
        <div className="space-y-5">
          <div><h2 className="text-[15px] font-medium text-[#ECECEE]">Agent access for new servers</h2><p className="mt-1 text-xs text-[#77777F]">Applies when you click Add server. Use the agent chips on each server card to change access at any time — the agent picks it up on its next message.</p><div className="mt-3 space-y-2">{bots.map((bot) => <label key={bot.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-[#292930] bg-[#101012] px-3 py-3"><input type="checkbox" checked={selectedBotIds.includes(bot.id)} onChange={() => toggleBot(bot.id)} className="accent-[#7785FF]" /><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#30356A] text-xs text-[#E2E4FF]">{bot.name.slice(0, 1).toUpperCase()}</span><span><span className="block text-sm text-[#E4E4E7]">{bot.name}</span><span className="block text-xs text-[#77777F]">{bot.title}</span></span></label>)}</div></div>
          <div><h2 className="text-[15px] font-medium text-[#ECECEE]">Configured servers</h2>{error ? <p className="mt-2 rounded-xl border border-[#6A2C37] bg-[#2A151A] p-3 text-xs text-[#F3A2AA]">{error}</p> : null}<div className="mt-3 space-y-2">{servers.length === 0 ? <p className="rounded-xl border border-dashed border-[#34343B] p-5 text-sm text-[#77777F]">No MCP servers yet.</p> : servers.map((server) => <div key={server.id} className="rounded-xl border border-[#292930] bg-[#101012] p-4"><div className="flex items-center justify-between"><span className="font-medium text-[#ECECEE]">{server.name}</span><span className="rounded-full bg-[#202536] px-2 py-1 text-[10px] uppercase text-[#AEB7FF]">{server.transport.replace("_", " ")}</span></div><p className="mt-1 text-xs text-[#77777F]">{server.endpoint ?? server.command ?? server.slug}</p><p className={`mt-2 text-[11px] ${server.oauthStatus === "reconnect" ? "text-[#F0A15A]" : "text-[#6E778A]"}`}>{server.oauthStatus === "connected" ? "OAuth connected" : server.oauthStatus === "reconnect" ? "Authorization expired — reconnect required" : server.hasSecret ? "Encrypted static credential saved" : "No credential saved"}</p><div className="mt-3 flex flex-wrap items-center gap-1.5"><span className="text-[11px] text-[#77777F]">Agents:</span>{bots.map((bot) => { const assigned = (botAssignments[bot.id] ?? []).some((entry) => entry.serverId === server.id); return <button key={bot.id} type="button" onClick={() => void toggleAssignment(server, bot.id)} className={`rounded-full border px-2.5 py-1 text-[11px] ${assigned ? "border-[#7785FF] bg-[#30356A] text-[#E2E4FF]" : "border-[#34343B] text-[#85858B]"}`}>{assigned ? "✓ " : ""}{bot.name}</button>; })}</div><div className="mt-3 flex flex-wrap gap-2">{server.transport !== "stdio" ? <><button type="button" disabled={oauthPending === server.id} onClick={() => void connectOAuth(server)} className="rounded-lg bg-[#7785FF] px-3 py-2 text-xs font-semibold text-[#090A12] disabled:opacity-50">{oauthPending === server.id ? "Connecting…" : server.oauthStatus === "none" ? "Connect OAuth" : "Reconnect OAuth"}</button>{server.oauthStatus !== "none" ? <button type="button" disabled={oauthPending === server.id} onClick={() => void disconnectOAuth(server)} className="rounded-lg border border-[#34343B] px-3 py-2 text-xs text-[#B9B9C0]">Disconnect</button> : null}</> : null}<button type="button" onClick={() => void deleteServer(server)} className={`ml-auto rounded-lg border px-3 py-2 text-xs ${confirmingDelete === server.id ? "border-[#B4434F] bg-[#3A1A20] text-[#F3A2AA]" : "border-[#34343B] text-[#B9B9C0]"}`}>{confirmingDelete === server.id ? "Confirm delete" : "Delete"}</button></div></div>)}</div></div>
        </div>
      </div>
    </section>
  </div>;
